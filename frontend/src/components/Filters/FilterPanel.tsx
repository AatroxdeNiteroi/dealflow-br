...
function fmtAnos(v: number): string {
...
export default function FilterPanel({ domains, value, onChange, resultsTotal }: Props) {
  ...
  function applyQuickFilter(preset: "sweet" | "midmarket" | "startups" | "indústria") {
    ...
    <aside className="sidebar">
      ...
      <div className="filters-status">
        <span>
          ...
          {activeCount === 0 ? "Sem filtros" : `${activeCount} filtro${activeCount > 1 ? "s" : ""}`}
        </span>
      ...
      <div className="filter-group">
        <div className="filter-label">Busca</div>
        <div className="search-wrap">
          ...
          {value.search && (
            <button
              ...
              onClick={() => onChange({ ...value, search: undefined, offset: 0 })}
              title="limpar busca"
            >×</button>
          )}
        </div>
      ...
      <div className="filter-group">
        ...
        <div className="quick-filters">
          ...
          <button className="quick-btn" onClick={() => applyQuickFilter("indústria")}>
      ...
      <div className="filter-group">
        <div className="filter-label">Receita estimada</div>
        <DualRangeSlider
          min={1_000_000}
          max={250_000_000}
          step={500_000}
          scale="log"
          valueMin={value.receita_min_brl}
          valueMax={value.receita_max_brl}
      ...
      <div className="filter-group">
        <div className="filter-label">Headcount · CLT</div>
        <DualRangeSlider
          min={domains.ranges.headcount.min}
          max={Math.min(domains.ranges.headcount.max, 3000)}
          step={5}
          valueMin={value.headcount_min}
          valueMax={value.headcount_max}
      ...
      <div className="filter-group">
        <div className="filter-label">Idade da empresa</div>
        <DualRangeSlider
          min={0}
          max={Math.min(domains.ranges.idade_empresa.max, 100)}
          step={1}
          valueMin={value.idade_min}
          valueMax={value.idade_max}
      ...
      <div className="filter-group">
        <div className="filter-label">Capital social</div>
        <DualRangeSlider
          min={1_000}
          max={50_000_000}
          step={1_000}
          scale="log"
          valueMin={value.capital_min_brl}
          valueMax={value.capital_max_brl}
      ...
      <div className="filter-group">
        ...
        <DualRangeSlider
          min={0}
          max={Math.min(domains.ranges.n_socios.max, 30)}
          step={1}
          valueMin={value.n_socios_min}
          valueMax={value.n_socios_max}
